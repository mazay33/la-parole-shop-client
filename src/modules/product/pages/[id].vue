...
watchEffect(() => {
	if (product.value?.cupSizes.length) {
		selectedCupSize.value = product.value?.cupSizes[0].id;
	}

	if (product.value?.clothingSizes.length) {
	...
	if (product.value?.beltSizes.length) {
...
const isAddToCartDisabled = computed(() => {
	...
	if (product.value.productConfigurations.length && !selectedConiguration.value) return true;
	if (selectedConiguration.value?.id !== product.value?.productConfigurations[0]?.id && !selectedBeltSize.value)
		return true;
});
...
const addToCart = async () => {
	...
	if (
		product.value.productConfigurations &&
		selectedConiguration.value?.id === product.value.productConfigurations[0].id
	) {
		selectedBeltSize.value = undefined;
	}
	...
	<div>
		<div class="grid grid-cols-2 gap-10 font-['Raleway'] text-lg">
			...
			<div class="flex flex-col justify-start mx-5">
				...
				<div class="pt-10">
					Комплект из графитового кружева с перламутровой цветочной вышивкой. Бюстгальтер балконет на
					косточках с мягкой чашкой, регулируемые бретели, застежка на крючки. Трусики бразильяно регулируемые
					по бокам. Пояс возможно носить как с чулками, так и с гартерами.
				</div>
			</div>
			<slider
